...
type VnEditorTab = 'specialized' | 'all_data';
...
export const VnEditor: React.FC<VnEditorProps> = ({ data, onChange }) => {
  ...
  const hasSpecializedData = storyFlags.length > 0 || Object.keys(loveInterestPoints).length > 0 || data?.playerName !== undefined || data?.currentSceneId !== undefined;

  const [activeTab, setActiveTab] = useState<VnEditorTab>(hasSpecializedData ? 'specialized' : 'all_data');
    ...
    <div>
        <div className="border-b border-gray-600 mb-6">
            <nav className="-mb-px flex space-x-6">
                {hasSpecializedData && <button onClick={() => setActiveTab('specialized')} className={`py-3 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'specialized' ? 'border-blue-500 text-blue-400' : 'border-transparent text-gray-400 hover:text-gray-200 hover:border-gray-500'}`}>Specialized Editor</button>}
        ...
        {activeTab === 'specialized' && hasSpecializedData && (
        ...
        {activeTab === 'all_data' && (
            <GenericJsonEditor data={data} onChange={onChange} />
        )}
    </div>
  );
};